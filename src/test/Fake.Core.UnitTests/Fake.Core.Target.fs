...
let validateBuildOrder a b =
    ignore a
    ignore b

let testCaseMultipleRuns name f =
    [ Fake.ContextHelper.fakeContextTestCase (sprintf "%s - run" name)
      <| fun c -> f run c
      Fake.ContextHelper.fakeContextTestCase (sprintf "%s - runParallel" name)
      <| fun c -> f runParallel c ]

type TestTarget =
    | TestTarget of string
    | TestTargetGroup of TestTarget list

    member x.TargetNames =
        seq {
            match x with
...
let tests =
    testList
        "Fake.Core.Target.Tests"
        ([ Fake.ContextHelper.fakeContextTestCaseAssertTime (TimeSpan.FromSeconds 18.0) "basic performance #2036"
           <| fun _ ->
               let counter = 2500
               CoreTracing.setTraceListeners [] // silence

               let all_Pipelines =
                   Dictionary<string, TestTarget>(System.StringComparer.OrdinalIgnoreCase :> IEqualityComparer<string>)
               ...
               let t_targets =
                   TestTargetGroup [ for i in 0..counter -> TestTarget.create (sprintf "Target_%i" i) ignore ]

               [ t_targets ] |> CreatePipeline "Run.1"
               //Target.printDependencyGraph true "Run.1"
               Target.runOrDefaultWithArguments "Run.1"

           Fake.ContextHelper.fakeContextTestCaseAssertTime (TimeSpan.FromSeconds 13.0) "basic performance (1)"
           <| fun _ ->
               let counter = 5000
               CoreTracing.setTraceListeners [] // silence
           ...
           Fake.ContextHelper.fakeContextTestCase "handle casing in target runner"
           <| fun _ ->
               ...
               let ctx =
                   TargetContext.Create "cc" targets [] System.Threading.CancellationToken.None
           ...
           <| fun _ ->
               ...
               | [ TargetSet [ "a"; "b"; "c" ]; [| Target "dep" |] ] ->
                   // as expected
                   ()
               | _ -> Expect.isTrue false (sprintf "inconsistent order: %A" order)

           Fake.ContextHelper.fakeContextTestCase "issue #1395 example"
           <| fun _ ->
           ...
           Fake.ContextHelper.fakeContextTestCase "casing in targets - #2000"
           <| fun _ ->
               ...
               ("CleanUp" ==> "RunUnitTests" ==> "RunAlltests" ==> "Default") |> ignore

               [ "CleanUp"; "RunUnitTests"; "RunAlltests"; "Default" ]
               |> Seq.iter (fun i ->
                   let t = i |> Target.get
           ...
           Fake.ContextHelper.fakeContextTestCase "Diamonds are resolved correctly"
           <| fun _ ->
           ...
           Fake.ContextHelper.fakeContextTestCase "Initial Targets Can Run Concurrently"
           <| fun _ ->
               Target.create "a" DoNothing
               Target.create "b" DoNothing
           ...
           Fake.ContextHelper.fakeContextTestCase "BlythMeisters Scenario Of Complex Build Order Is Correct"
           <| fun _ ->
           ...
           Fake.ContextHelper.fakeContextTestCase "BlythMeisters Scenario Of Even More Complex Build Order Is Correct"
           <| fun _ ->
               Target.create "PrepareBuild" DoNothing
               Target.create "CreateWholeCaboodle" DoNothing
               Target.create "UpdateVersions" DoNothing
           ...
           Fake.ContextHelper.fakeContextTestCase "Spurs run as early as possible"
           <| fun _ ->
               Target.create "a" DoNothing
               Target.create "b" DoNothing
               Target.create "c1" DoNothing
           ...
           Fake.ContextHelper.fakeContextTestCase "Spurs run as early as possible 3 and 2 length"
           <| fun _ ->
               Target.create "a" DoNothing
           ...
           Fake.ContextHelper.fakeContextTestCase "Spurs run as early as possible (reverse definition order)"
           <| fun _ ->
               Target.create "a" DoNothing
               Target.create "b" DoNothing
               Target.create "c1" DoNothing
           ...
           Fake.ContextHelper.fakeContextTestCase "Spurs run as early as possible split on longer spur"
           <| fun _ ->
               Target.create "a" DoNothing
               Target.create "b" DoNothing
               Target.create "c1" DoNothing
           ...
           Fake.ContextHelper.fakeContextTestCase "3 way Spurs run as early as possible"
           <| fun _ ->
               Target.create "a" DoNothing
               Target.create "b" DoNothing
               Target.create "c1" DoNothing
           ...
           Fake.ContextHelper.fakeContextTestCase "Soft dependencies are respected when dependees are present"
           <| fun _ ->
               Target.create "a" DoNothing
               Target.create "b" DoNothing
               Target.create "c" DoNothing
           ...
           Fake.ContextHelper.fakeContextTestCase "Soft dependencies are ignored when dependees are not present"
           <| fun _ ->
               Target.create "a" DoNothing
               Target.create "b" DoNothing
               Target.create "c" DoNothing
           ...
           Fake.ContextHelper.fakeContextTestCase "Fsharp.Data Dependencies single worker (broken)"
           <| fun _ ->
           ...
           Fake.ContextHelper.fakeContextTestCase "Test that we run a simple target with dependency"
           <| fun _ ->
           ...
           Fake.ContextHelper.fakeContextTestCase "Test we output targets after failing targets"
           <| fun _ ->
               Target.create "SimpleTest" ignore
           ...
           Fake.ContextHelper.fakeContextTestCase
               "Running order resolves quickly when there are numerous targets with numerous dependencies"
           <| fun _ ->
               ...
               "Chaetopoda"
               <== [ "Latium"; "comparableness"; "exasperating"; "matriarches"; "mediatise" ]
               ...
               "Merrie"
               <== [ "Latium"; "exasperating"; "glorified"; "matriarches"; "mediatise" ]
               ...
               "carp-"
               <== [ "Latium"; "Vietcong"; "exasperating"; "mediatise"; "ungeuntarium" ]
               ...
               "clingstones"
               <== [ "comparableness"; "exasperating"; "mediatise"; "ungeuntarium" ]
               ...
               "conspiratorially"
               <== [ "begnawn"; "druggeries"; "hepatectomies"; "interproximate"; "relacing" ]

               "contin" <== [ "exasperating"; "mnemonicalist"; "ungeuntarium" ]

               "contrivement"
               <== [ "Vietcong"; "exasperating"; "mediatise"; "mnemonicalist"; "ungeuntarium" ]
               ...
               "hyeniform"
               <== [ "Latium"; "Vietcong"; "exasperating"; "mnemonicalist"; "ungeuntarium" ]
               ...
               "hypermoralistic"
               <== [ "clingstones"; "exasperating"; "mediatise"; "ungeuntarium" ]
               ...
               "hypozoic"
               <== [ "Latium"; "exasperating"; "glorified"; "muensters"; "ungeuntarium" ]
               ...
               "marlinespike"
               <== [ "Danelage"; "Lookeba"; "exasperating"; "mediatise"; "ungeuntarium" ]
               ...
               "mnemonicalist"
               <== [ "Latium"; "Latium"; "exasperating"; "exasperating"; "ungeuntarium" ]
               ...
               "orchiocele"
               <== [ "beesting"; "cometography"; "exasperating"; "marlinespike"; "mediatise" ]
               ...
               "overanalyze"
               <== [ "comparableness"; "exasperating"; "mediatise"; "uninnocent" ]
               ...
               "potto"
               <== [ "Phascum"; "begnawn"; "ichthyol."; "interproximate"; "leered"; "relacing" ]

               "prefearful"
               <== [ "comparableness"; "exasperating"; "komatiks"; "ungeuntarium" ]
               ...
               "sclerodactylia"
               <== [ "comparableness"; "compresent"; "exasperating"; "mediatise" ]
               ...
               "surrendry"
               <== [ "Vietcong"; "begnawn"; "cometography"; "relacing"; "ungeuntarium" ]
               ...
               "thoracoabdominal"
               <== [ "collection"; "exasperating"; "mediatise"; "papyrographic" ]
               ...
               "ungeuntarium"
               <== [ "Vietcong"; "cometography"; "cometography"; "exasperating"; "mediatise" ]
               ...
               "voter"
               <== [ "begnawn"; "ichthyol."; "interproximate"; "outdoorness"; "ungeuntarium" ]
               ...
               if elapsedSeconds > 10 then
                   failwithf "Unexpectedly long time to complete (%d seconds); should only take a second" elapsedSeconds
               ...
               | _ -> failwithf "unexpected order: %A" (order |> List.map (Array.map (fun t -> t.Name))) ]
         @ ([ testCaseMultipleRuns "Not activated final target does not run"
              <| fun myRun _ ->
                  Target.create "a" ignore
                  Target.create "b" ignore
                  Target.create "c" ignore
                  ...
                  let context = myRun "c"
                  let actualOrder = context.PreviousTargets |> List.map (fun tr -> tr.Target.Name)
              ...
              testCaseMultipleRuns "Final targets run after all targets"
              <| fun myRun _ ->
                  Target.create "a" DoNothing
                  Target.create "b" DoNothing
                  Target.create "c" DoNothing
              ...
              testCaseMultipleRuns "BuildFailure targets do not run if nothing fails"
              <| fun myRun _ ->
                  Target.create "a" ignore
                  Target.create "b" ignore
                  Target.create "c" ignore
              ...
              testCaseMultipleRuns "BuildFailure targets do not run if not activated"
              <| fun myRun _ ->
                  Target.create "a" ignore
              ...
              testCaseMultipleRuns "BuildFailure targets run after failing targets"
              <| fun myRun _ ->
                  Target.create "a" DoNothing
                  Target.create "b" (fun _ -> failwith "failed dependency")
                  Target.create "c" DoNothing
                  ...
                  let bResult = context.PreviousTargets |> List.find (fun tr -> tr.Target.Name = "b")
                  let cResult = context.PreviousTargets |> List.find (fun tr -> tr.Target.Name = "c")
                  Expect.equal true context.HasError "Expected failure"
              ...
              testCaseMultipleRuns "Final targets run after failing targets"
              <| fun myRun _ ->
                  Target.create "a" DoNothing
                  Target.create "b" (fun _ -> failwith "failed dependency")
                  Target.create "c" DoNothing
