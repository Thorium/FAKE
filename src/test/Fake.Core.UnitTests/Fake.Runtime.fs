...
let tests =
    testList
        "Fake.Runtime.Tests"
        [ testCase "Test that cache helpers work"
          <| fun _ ->
          ...
          testCase "CoreCache.findInAssemblyList works with null token"
          <| fun _ ->
              let toFind =
                  AssemblyName "Microsoft.ServiceFabric.Client.Http, Culture=neutral, PublicKeyToken=null"

              let res =
                  { FullName =
                      "Microsoft.ServiceFabric.Client.Http, Version=3.0.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35"
                    Version = "3.0.0.0"
                    Location = "C:\\invalid.dll" }
          ...
          testCase "CoreCache.findInAssemblyList works with different version"
          <| fun _ ->
              let toFind =
                  AssemblyName
                      "Microsoft.ServiceFabric.Client.Http, Version=2.0.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35"
          ...
          testCase "CoreCache.findInAssemblyList doesn't return different token"
          <| fun _ ->
              let toFind =
                  AssemblyName
                      "Microsoft.ServiceFabric.Client.Http, Version=2.0.0.0, Culture=neutral, PublicKeyToken=41bf3856ad364e35"
          ...
          testCase "CoreCache.findInAssemblyList doesn't return different name"
          <| fun _ ->
              let toFind =
                  AssemblyName
                      "Microsoft.ServiceFabric.Client, Version=3.0.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35"
          ...
          testCase "Test that cache helpers work for nuget cache"
          <| fun _ ->
              let nugetLib =
                  Paket.Constants.UserNuGetPackagesFolder </> "MyLib" </> "lib" </> "mylib.dll"
          ...
          testCase "Test that we can properly find type names when the file name contains '.'"
          <| fun _ ->
          ...
          testCase "Test that we can tokenize __SOURCE_FILE__"
          <| fun _ ->
              // Add test if everything works with __SOURCE_FILE__
          ...
          testCase "Test that we find the correct references"
          <| fun _ ->
              let scriptText =
                  """
...
nuget Fake.Core.SemVer prerelease //"
      ...
      """
              ...
              let expected =
                  [ Fake.Runtime.FSharpParser.InterestingItem.Reference(
                        sprintf "paket:\nnuget Fake.Core.SemVer prerelease //"
                    ) ]
          ...
          // Add test if everything works with #ifdefed #r "paket: line"
          testCase "Test that we find the correct references without defines"
          <| fun _ ->
              let scriptText =
                  """
#if BOOTSTRAP && DOTNETCORE
...
nuget Fake.Core.SemVer prerelease //"
      ...
      """
          ...
          testCase "Test #1947 - non-existing folders"
          <| fun _ ->
              ...
              try
                  Directory.CreateDirectory(tmpDir </> "packages" </> "Octokit" </> "lib" </> "net45")
                  |> ignore
...
"""
          ...
          testCase "Test #1947 - test #I with ."
          <| fun _ ->
              let tmpDir = Path.GetTempFileName()
              File.Delete tmpDir
              Directory.CreateDirectory tmpDir |> ignore
...
"""
          ...
          testCase "Test #1947 - good error message"
          <| fun _ ->
              let tmpDir = Path.GetTempFileName()
              File.Delete tmpDir
              Directory.CreateDirectory tmpDir |> ignore
...
"""
                  ...
                  with e ->
                      (e.Message.Contains "test.fsx(2,1): error FS2302: Directory '"
