...
let printVersion () =
    traceFAKE
        "%s (this line is written to standard error, see https://github.com/fsharp/FAKE/issues/2066)"
        fakeVersionStr

let printFakePath () = traceFAKE "FakePath: %s" fakePath
...
let printEnvironment cmdArgs args =
    ...
    if cmdArgs |> Array.length > 1 then
        ...
        args |> Seq.map fst |> Seq.iter (tracefn "%A")

    log ""
//traceFAKE "FSI-Path: %s" fsiPath
//traceFAKE "MSBuild-Path: %s" msBuildExe

let containsParam (param: string) =
    Seq.map (fun (s: string) -> s.ToLower()) >> Seq.exists ((=) (param.ToLower()))

let paramIsHelp param =
    containsParam param [ "help"; "?"; "/?"; "-h"; "--help"; "/h"; "/help" ]

let buildScripts =
    System.IO.Directory.EnumerateFiles(System.IO.Directory.GetCurrentDirectory(), "*.fsx")
    |> Seq.toList
...
let trimMatchingQuotes quote (s: string) =
    if s.Length >= 2 && s.[0] = quote && s.[s.Length - 1] = quote then
...
let reportExn (verb: VerboseLevel) exn =
    use logPaket =
        ...
        if verb = Trace.Silent then
            Paket.Logging.event.Publish |> Observable.subscribe Paket.Logging.traceToConsole
        else
    ...
    if Environment.GetEnvironmentVariable "FAKE_DETAILED_ERRORS" = "true" then
        ...
        Paket.Logging.printErrorExt verb.PrintVerbose verb.PrintVerbose verb.PrintVerbose exn
...
let runOrBuild (args: RunArguments) =
    ...
    try
        ...
        let additionalArgs, scriptFile, scriptArgs =
            match s, fsiArgs |> Array.toList, List.isEmpty buildScripts with
            ...
            | Some (script), fsArgs, _ ->
                ...
                | Some (i) ->
                    ...
                    if script <> fsxPath then
                        traceFAKE "script specified via fsiargs '%s' does not equal the one we run '%s'." fsxPath script

                    let fsiOpts = if i > 0 then fsiArgs.[0 .. i - 1] else [||]

                    let scriptArgs =
                        if fsiArgs.Length > (i + 1) then
                            fsiArgs.[i + 1 ..]
                        else
                            [||]
            ...
            | None, x :: xs, _ ->
                ...
                | Some (i) ->
                    ...
                    if fsxPath.EndsWith(".fsx", StringComparison.OrdinalIgnoreCase) then
                        ...
                        let msg =
                            sprintf
                                "Expected argument %s to be the build script path, but it does not have the .fsx extension."
                                fsxPath
            ...
            | None, [], true ->
                failwith "Build script not specified on command line, in fsi args or found in working directory."

        if scriptArgs <> [] then
            traceFAKE "FAKE 6 ignores script args given via --fsiargs, given was %A" scriptArgs

        let useCache = not args.NoCache

        try
            let config =
                FakeRuntime.createConfigSimple
                    args.VerboseLevel
                    additionalArgs
                    scriptFile
                    args.ScriptArguments
                    useCache
                    args.RestoreOnlyGroup
            ...
            let result =
                ...
                | Runners.RunResult.CompilationError err ->
                    let indentString num (str: string) =
                        ...
                        indentString
                        + String.Join(sprintf "%s%s" Environment.NewLine indentString, splitMsg)

                    if args.VerboseLevel.PrintVerbose then
                        // in case stderr is not redirected
                        TraceMessage("Script is not valid, see standard error for details.", true)
                        |> forceWrite
                ...
                | Runners.RunResult.RuntimeError err ->
                    if args.VerboseLevel.PrintVerbose then
                        // in case stderr is not redirected
                        TraceMessage("Script reported an error, see standard error for details.", true)
                        |> forceWrite
            ...
            if
                args.VerboseLevel.PrintNormal
                && Environment.GetEnvironmentVariable "FAKE_DISABLE_HINTS" <> "true"
            then
    ...
    with exn ->
        if args.VerboseLevel.PrintVerbose then
            // in case stderr is not redirected
            TraceMessage("There was a problem while setting up the environment, see standard error for details.", true)
            |> forceWrite

        ErrorMessage "There was a problem while setting up the environment:"
        |> forceWrite
...
let parseAction (results: DocoptMap) =
    let verbLevel = DocoptResult.getFlagCount "--verbose" results
    let isSilent = DocoptResult.hasFlag "--silent" results
    ...
    if DocoptResult.hasFlag "--version" results then
        ...
        let arg =
            { Script =
                if isRun then
                    DocoptResult.tryGetArgument "<script.fsx>" results
                else
                    DocoptResult.tryGetArgument "--script" results
              ScriptArguments =
              ...
              RestoreOnlyGroup =
                DocoptResult.hasFlag "--partial-restore" results
                || Environment.GetEnvironmentVariable("FAKE_PARTIAL_RESTORE") = "true"
              VerboseLevel = verboseLevel
...
let main (args: string[]) =
    let resolution =
        Environment.GetEnvironmentVariable("PAKET_DISABLE_RUNTIME_RESOLUTION")
    ...
    let paketPerfTask =
        async {
            ...
            with e ->
                eprintfn "Building paket performance maps failed: %O" e
        }
        |> Async.StartAsTask
